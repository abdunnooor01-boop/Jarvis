...
class ApiClient {
  ...
  async markKnowledgeEntryRead(id: string): Promise<any> {
    return this.request<any>('POST', `/knowledge/entries/${id}/read`);
  }
  ...
  async createTestPlan(plan: any): Promise<TestPlan> {
    return this.request<TestPlan>('POST', '/testing/plans', plan);
  }

  async editTestPlan(id: string, plan: any): Promise<TestPlan> {
    return this.request<TestPlan>('PATCH', `/testing/plans/${id}`, plan);
  }

  async triggerTestPlanRun(planId: string): Promise<any> {
    return this.request<any>('POST', `/testing/plans/${planId}/run`);
  }

  async getTestingSubscription(): Promise<any> {
    return this.request<any>('GET', '/testing/subscription');
  }

  async subscribeToTesting(tier: string): Promise<any> {
    return this.request<any>('POST', '/testing/subscription', { tier });
  }

  // ==================== Knowledge Sources ====================

  async getKnowledgeSources(): Promise<any> {
    return this.request<any>('GET', '/knowledge/sources');
  }

  async addKnowledgeSource(data: { name: string; url: string; category: string; description?: string }): Promise<any> {
    return this.request<any>('POST', '/knowledge/sources', data);
  }

  async refreshKnowledgeSource(id: string): Promise<any> {
    return this.request<any>('POST', `/knowledge/crawl/${id}`);
  }

  // ==================== Notifications ====================

  async registerDeviceToken(data: { token: string; platform: string; device_name?: string | null }): Promise<any> {
    return this.request<any>('POST', '/notifications/register', data);
  }
  ...
  async updateNotificationPreferences(data: any): Promise<any> {
    return this.request<any>('PUT', '/notifications/preferences', data);
  }
